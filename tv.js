(function () {
    ...
    // Безпечне збереження оригінальної фільтрації навігатора замість її повного перезапису
...
`;
    ...
    // Безпечне оновлення CSS-стилів (усуває дублювання <style> в DOM)
    ...
    var plugTvKeys = {
        _listenTvKeys: function (e) {
            // Ігноруємо натискання, якщо відкритий вибір джерел, модальне вікно чи випадаюче меню
            ...
            // Чітка прив'язка кольорів до класів кнопок
        ...
        _reBindRight: function (e) {
            if (e.name === 'content' && Lampa.Controller.enabled().controller) {
                // Не перехоплюємо правий клик, якщо працює вибір джерела
                if (document.querySelector('.select') || document.querySelector('.modal')) return;
        ...
        start: function () {
            document.removeEventListener('keyup', this._listenTvKeys);
            if (Lampa.Storage.get('apx_colorButton', false) === true) {
                document.addEventListener('keyup', this._listenTvKeys.bind(this));
            }
